...
from util import *
from vec2graph import GraphGenerator
...
def train_model_ver2(train_loader, test_loader, tokenizer, args):
...
#    vec2seq = Vec2SeqPeeky(tokenizer, args.emb_size, args.hidden_size, args.latent_vec_size, args.num_layers, args.dropout).to(get_device())
    vec2seq = GraphGenerator(tokenizer, args.emb_size, args.hidden_size, args.num_layers, args.dropout).to(get_device())
...
#    num_samples = min(args.num_samples, len(test_loader.dataset))
    ...
    novelty, novel_dfs_codes = calculate_novelty(generated_graphs, train_loader)
    print("Novelty: {:.2f}%".format(novelty))
    uniqueness, unique_dfs_codes, molecule_counts = calculate_uniqueness(generated_graphs)
    print("Uniqueness: {:.2f}%".format(uniqueness))

    diversity = calculate_diversity(generated_graphs, tokenizer, args.wl_iteration)
    print("Diversity: {:.2f}%".format(diversity))   

    analyzer = GraphAnalyzer()
    stats =analyzer.analyze_graph_structure(generated_graphs)

        # 分析結果の表示
#    print("グラフ構造の分析結果:")
        # 可視化を含む分析
#        stats = analyzer.analyze_and_visualize_graphs(
#            generated_graphs,
...
#        )
    ...
    results = evaluate_generated_graphs(reference_graphs, nx_graphs, sigma=1.0)

    print("==== MMD Results ====")
    print(f"Degree MMD:      {results['degree']:.4f}")
    print(f"Clustering MMD:  {results['clustering']:.4f}")
    print(f"Orbit MMD(4-node motif, 11 classes): {results['orbit']:.4f}")
...
#
#        # グラフメトリクスの評価を実行
#        evaluation_results = metrics.evaluate_graphs(nx_graphs, reference_graphs)
#
#
#        # 評価結果の表示
#        print("\nGraph Metrics Evaluation Results:")
...
#        minimam_dfs_codes = []
#        for dfs_codes in decoded_dfs_codes:
#            if check_minimality(dfs_codes):
#                minimam_dfs_codes.append(dfs_codes)

#        minimam_dfs_codes_to_dfs_codes_ratio = len(minimam_dfs_codes) / len(decoded_dfs_codes)

#        total_similarity = 0.0
#        counter = 0.0
#        for id, dfs_codes in enumerate(minimam_dfs_codes):
#            data_dfs = test_loader.dataset[id][1]
#            total_similarity += compare.comp_cosine_similarity(dfs_codes, data_dfs)
#            counter += 1

#        average_similarity_to_minimam_dfs_codes = total_similarity / counter

#        print("Epoch: {}, Loss: {:.2f}, average similarity: {:.2f}, minimam_dfs_codes_to_dfs_codes_ratio: {:.2f}, average_similarity_to_minimam_dfs_codes: {:.2f}".format(epoch, total_loss, average_similarity, minimam_dfs_codes_to_dfs_codes_ratio, average_similarity_to_minimam_dfs_codes))

#        print("Epoch: {}, Loss: {:.2f}".format(epoch, total_loss))

#        avg_valid_loss = total_valid_loss / len(test_loader)
#        print("Epoch: {}, Loss: {}, Validation Loss: {}".format(epoch, total_loss, avg_valid_loss))
...
def main():
    ...
    trained_model = train_model_ver2(train_loader, test_loader, tokenizer, args)
